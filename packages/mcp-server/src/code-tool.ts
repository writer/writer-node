...
async function run(client) {
  const chatCompletion = await client.chat.chat({ messages: [{ content: 'Write a haiku about programming', role: 'user' }], model: 'palmyra-x5' });
...
const remoteStainlessHandler = async (
  ...
  const localClientEnvs = { WRITER_API_KEY: requireValue(readEnv('WRITER_API_KEY') ?? client.apiKey, 'set WRITER_API_KEY environment variable or provide apiKey client option'), WRITER_BASE_URL: readEnv('WRITER_BASE_URL') ?? client.baseURL ?? undefined };
