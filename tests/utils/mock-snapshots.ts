import Writer from 'writer-sdk/index';
import { RequestInfo } from 'writer-sdk/internal/builtin-types';
...
export async function makeSnapshotRequest<T>(
  requestFn: (client: Writer) => Promise<T>,
  snapshotIndex = 1,
): Promise<T> {
  if (process.env['UPDATE_API_SNAPSHOTS'] === '1') {
    ...
    const writer = new Writer({ fetch });

    const result = await requestFn(writer);
  ...
  const writer = new Writer({ fetch, apiKey: 'My API Key' });
  const requestPromise = requestFn(writer);
...
export async function makeStreamSnapshotRequest<T extends AsyncIterable<any>>(
  ...
  if (process.env['UPDATE_API_SNAPSHOTS'] === '1') {
    ...
    const iterator = requestFn(writer);
