import { APIUserAbortError, WriterError } from '../error';
...
export class EventStream<EventTypes extends BaseEvents> {
  ...
  #rejectConnectedPromise: (error: WriterError) => void = () => {};

  #endPromise: Promise<void>;
  #resolveEndPromise: () => void = () => {};
  #rejectEndPromise: (error: WriterError) => void = () => {};
  ...
  #handleError(this: EventStream<EventTypes>, error: unknown) {
    ...
    if (error instanceof Error) {
      const writerError: WriterError = new WriterError(error.message);
      // @ts-ignore
      writerError.cause = error;
      return this._emit('error', writerError);
    }
    return this._emit('error', new WriterError(String(error)));
  }
  ...
  _emit<Event extends keyof EventTypes>(
    ...
    if (event === 'error') {
      ...
      const error = args[0] as WriterError;
      if (!this.#catchingPromiseCreated && !listeners?.length) {
